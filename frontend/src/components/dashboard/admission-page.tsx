    ...
    >
      <div className="max-w-4xl mx-auto">
        <div className="rounded-[2.5rem] border border-[var(--border)] bg-[var(--bg-secondary)] overflow-hidden shadow-[0_24px_70px_rgba(15,23,42,0.05)]">
          ...
          {/* Header Area */}
          <div className="bg-[var(--bg-primary)] border-b border-[var(--border)] px-10 py-8">
             <div className="flex items-center justify-between">
                <div>
                   <h3 className="text-xl font-bold text-[var(--text-primary)]">Student Enrollment</h3>
            ...
            <AnimatePresence mode="wait">
              {step === 1 && (
                ...
                >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    />
                    <div>
                      <label className="block text-xs font-bold text-[var(--text-muted)] uppercase tracking-widest mb-2">Gender</label>
                      <select 
                  ...
                  <div className="pt-4 border-t border-[var(--border)]">
                    <h4 className="font-bold text-[var(--text-primary)] mb-6">Parent/Guardian Information</h4>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <InputField 
                        label="Guardian Name" 
                ...
                >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                     <div className="space-y-4">
                        <label className="block text-xs font-bold text-[var(--text-muted)] uppercase tracking-widest">Select Grade</label>
                        <div className="grid grid-cols-3 gap-3">
                           {["Grade 8", "Grade 9", "Grade 10"].map((g) => (
                             <button 
                               key={g}
                               onClick={() => setFormData({...formData, grade: g})}
                               className={`py-3 rounded-xl border-2 font-bold text-sm transition-all ${formData.grade === g ? "border-[var(--accent)] bg-[var(--accent)]/5 text-[var(--accent)]" : "border-[var(--border)] text-[var(--text-muted)] hover:border-[var(--border)]"}`}
                             >
                               {g}
                             </button>
                           ))}
                     ...
                     <div className="space-y-4">
                        <label className="block text-xs font-bold text-[var(--text-muted)] uppercase tracking-widest">Select Section</label>
                        <div className="grid grid-cols-3 gap-3">
                           {["Section A", "Section B", "Section C"].map((s) => (
                             <button 
                               key={s}
                               onClick={() => setFormData({...formData, section: s})}
                               className={`py-3 rounded-xl border-2 font-bold text-sm transition-all ${formData.section === s ? "border-[var(--accent)] bg-[var(--accent)]/5 text-[var(--accent)]" : "border-[var(--border)] text-[var(--text-muted)] hover:border-[var(--border)]"}`}
                             >
                               {s}
                             </button>
                           ))}
              ...
              {step === 3 && (
                ...
                >
                  ...
                  <h3 className="text-3xl font-bold text-[var(--text-primary)] mb-4">Registration Successful</h3>
                  <p className="text-[var(--text-secondary)] max-w-sm mx-auto mb-10 leading-relaxed">
                    <span className="font-bold text-[var(--text-primary)]">{formData.firstName} {formData.lastName}</span> has been officially enrolled in {formData.grade} - {formData.section}.
                  </p>
